#!/usr/bin/env python3
"""
Simple fast pipeline runner for Dubai Hills property matching.

Processes files sequentially but uses optimized algorithms for speed.
"""
...
def main():
    """Main function to run the simple fast pipeline."""
    # Configure logging
    logger.add("logs/simple_fast_pipeline.log", rotation="1 day", retention="7 days")
    
    # Configuration
    owners_file = "data/raw/owners/20250716/Dubai Hills.xlsx - OLD.csv"
    transactions_dir = "data/raw/transactions/20250716"
    output_dir = "data/processed/simple_fast_pipeline"
    run_id = datetime.now().strftime('%Y%m%d_%H%M%S')
    
    logger.info("Starting simple fast pipeline processing")
    ...
    # Process files sequentially (but with fast algorithms)
    all_results = []
    
    for i, transaction_file in enumerate(transaction_files, 1):
        logger.info(f"Processing file {i}/{len(transaction_files)}")
        ...
        # Progress update
        if result:
            matches = result.get('data_volumes', {}).get('total_matches', 0)
            logger.info(f"File {i}/{len(transaction_files)} completed: {matches} matches")
        else:
            logger.warning(f"File {i}/{len(transaction_files)} failed")
    ...
    # Final summary
    logger.info("Simple fast pipeline processing completed!")
